...
})({"3yBaQ":[function(require,module,exports) {
var HMR_HOST = null;
var HMR_PORT = null;
var HMR_SECURE = false;
var HMR_ENV_HASH = "4a236f9275d0a351";
module.bundle.HMR_BUNDLE_ID = "987f42597eb25137";
"use strict";
...
},{}],"bj8Dy":[function(require,module,exports) {
...
class Colourous {
    ...
    randomColour() {
        return `rgb(${this._randomInt(0, 255)},${this._randomInt(0, 255)},${this._randomInt(0, 255)})`;
    }
    ...
    getHueList(color) {
        return color.slice(4, -1).split(`,`).map((val)=>Number(val)
        );
    }
    convertRGBToHex(colour) {
        const hueList = this.getHueList(colour);
        return `#${hueList.reduce((hex, cur)=>hex + this._convertDecimalToHex(cur)
        , ``)}`;
    }
    // Maths
    ...
    _calculateRelativeLuminance(colour) {
        const hueValues = colour.map((val)=>val <= 10 ? val / 3294 : (val / 269 + 0.0513) ** 2.4
        );
    ...
    calculateContrastRatio(colours) {
        ...
        if (typeof ((Math.max(...this.#luminosities) + 0.05) / (Math.min(...this.#luminosities) + 0.05)) !== `number`) console.log(`calcContrast`);
        return (Math.max(...this.#luminosities) + 0.05) / (Math.min(...this.#luminosities) + 0.05);
    }
    getOppositeColour(colour) {
        if (this._calculateRelativeLuminance(colour) > 0.5) return colour.map((val)=>Math.abs(225 - val)
        );
        else return colour.map((val)=>Math.abs(275 - val) > 255 ? 255 : Math.abs(275 - val)
        );
    }
    ...
    getDarkestContrastColour(contrasts) {
        const selectedColour = contrasts.sort((a, b)=>a[1] - b[1]
        ).find((c)=>c[1] > 7
        );
        if (!selectedColour) return contrasts.reduce((acc, c)=>c[1] > acc[1] ? c : acc
        , [
            [],
            0
        ])[0];
...
exports.default = new Colourous(); //#region Colour Functions
...
},{}]},["3yBaQ","bj8Dy"], "bj8Dy", "parcelRequiredd8e")

//# sourceMappingURL=index.7eb25137.js.map
